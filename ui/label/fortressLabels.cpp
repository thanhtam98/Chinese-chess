...
FortressLabels::FortressLabels(FWidget* parent) {
    ...
    fortressLines[0][1]->setGeometry(
        FPoint{(BOARD_WIDTH-4)*SPACE_BW_PIECE_X-1, 2}, 
        FSize{1,1}
    );
    ...
    fortressLines[0][2]->setGeometry(
        FPoint{(BOARD_WIDTH-4)*SPACE_BW_PIECE_X-1, SPACE_BW_PIECE_Y+2}, 
        FSize{1,1}
    );
    ...
    fortressLines[1][1]->setGeometry(
        FPoint{(BOARD_WIDTH-4)*SPACE_BW_PIECE_X-1, 7*SPACE_BW_PIECE_Y+2}, 
        FSize{1,1}
    );
    ...
    fortressLines[1][2]->setGeometry(
        FPoint{(BOARD_WIDTH-4)*SPACE_BW_PIECE_X-1, 8*SPACE_BW_PIECE_Y+2}, 
        FSize{1,1}
    );
